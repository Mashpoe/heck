...
typedef heck_stmt* (*stmt_copy)(heck_code*, heck_scope*,
				heck_stmt*); // int for number of indents
...
/* statement creation functions as well as vtable implementations have unique
 * naming conventions: for example, free_stmt_expr instead of stmt_expr_free.
 * this allows for better distinction between structures and faster
 * autocomplete, because there are so many statement types compared to other
 * types
 */
...
// BLOCK OF CODE
// block types are ordered from least to greatest precedence; do not change
// values/order
typedef enum
{
	BLOCK_DEFAULT = 0,
...
heck_stmt* create_stmt_block(heck_code* c, heck_file_pos* fp,
			     heck_block* block);
...
typedef struct heck_if_node
{
	...
	heck_block* code;
	struct heck_if_node*
	    next; // next node in linked list for if/else ladder
} heck_if_node;
