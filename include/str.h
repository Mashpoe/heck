...
/*	this structure is only compiler's representation of literals and
   identifiers, it has nothing to do with runtime. */
/*	these bois are supposed to be unique via a str_table, there shouldn't be
 * duplicate strings */
struct str_obj
{
	const char* value;
...
char* str_create(int* len, int* alloc,
		 const char* val); // val is optional; NULL or string
