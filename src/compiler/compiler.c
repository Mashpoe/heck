	...
	if (!func->imported)
	{
		...
		if (func->local_vec != NULL)
		{
			...
			vec_size_t num_params =
			    func->decl.param_vec == NULL
				? 0
				: vector_size(func->decl.param_vec);
			...
			for (int i = num_params; i < num_locals; ++i)
			{
				...
				wasm_str_lit(cmplr->wasm, " ");
				compile_data_type(
				    cmplr, func->local_vec[i]->data_type);
...
void compile_string_literals(heck_compiler* cmplr)
{
	...
	idf_map_iterate(cmplr->c->string_literals, compile_string_literal,
			cmplr);
}
...
const char* heck_compile(heck_code* c)
{
	...
	heck_compiler cmplr = {.wasm = wasm_code_create(),
			       .c = c,
			       .func_index = 0,
