...
void init_stmt(heck_file_pos* fp, heck_stmt* stmt, heck_stmt_type type,
	       const stmt_vtable* vtable);
inline void init_stmt(heck_file_pos* fp, heck_stmt* stmt, heck_stmt_type type,
		      const stmt_vtable* vtable)
{
	stmt->fp = fp;
	stmt->type = type;
...
heck_stmt* create_stmt_let(heck_code* c, heck_file_pos* fp,
...
heck_if_node* create_if_node(heck_code* c, heck_scope* parent,
			     heck_expr* condition)
{
...
heck_stmt* create_stmt_if(heck_code* c, heck_file_pos* fp,
			  heck_if_node* contents)
{
...
heck_stmt* create_stmt_while(heck_code* c, heck_file_pos* fp,
			     heck_expr* condition, heck_block* code)
{
...
heck_stmt* create_stmt_class(heck_code* c, heck_file_pos* fp,
...
heck_block* block_copy(heck_code* c, heck_scope* child, heck_block* block)
{
	...
	{
		vector_add(&new_block->stmt_vec,
			   copy_stmt(c, child, block->stmt_vec[i]));
	}
	return new_block;
}
...
{
	// store status in bool so we can continue resolving even when we come
	// across an error
...
const stmt_vtable stmt_vtable_expr = {resolve_stmt_expr, compile_stmt_expr,
				      copy_stmt_expr, print_stmt_expr};
...
const stmt_vtable stmt_vtable_let = {resolve_stmt_let, compile_stmt_let,
				     copy_stmt_let, print_stmt_let};
...
const stmt_vtable stmt_vtable_block = {resolve_stmt_block, compile_stmt_block,
				       copy_stmt_block, print_stmt_block};
...
const stmt_vtable stmt_vtable_if = {resolve_stmt_if, compile_stmt_if,
				    copy_stmt_if, print_stmt_if};
...
const stmt_vtable stmt_vtable_while = {resolve_stmt_while, compile_stmt_while,
				       copy_stmt_while, print_stmt_while};
...
const stmt_vtable stmt_vtable_ret = {resolve_stmt_ret, compile_stmt_ret,
				     copy_stmt_ret, print_stmt_ret};
...
const stmt_vtable stmt_vtable_class = {resolve_stmt_class, compile_stmt_class,
				       copy_stmt_class, print_stmt_class};
...
const stmt_vtable stmt_vtable_func = {resolve_stmt_func, compile_stmt_func,
				      copy_stmt_func, print_stmt_func};
...
const stmt_vtable stmt_vtable_err = {resolve_stmt_err, compile_stmt_err,
				     copy_stmt_err, print_stmt_err};
...
heck_stmt* copy_stmt_expr(heck_code* c, heck_scope* parent, heck_stmt* stmt)
{
	return create_stmt_expr(c, stmt->fp,
				copy_expr(c, stmt->value.expr_value));
}
...
heck_stmt* copy_stmt_block(heck_code* c, heck_scope* parent, heck_stmt* stmt)
{
	...
	// create child scope
	heck_scope* block_scope = scope_create(c, parent);
	return create_stmt_block(c, stmt->fp,
				 block_copy(c, block_scope, old_block));
}
...
{
	heck_stmt_while* while_stmt = &stmt->value.while_stmt;
	return create_stmt_while(c, stmt->fp,
				 copy_expr(c, while_stmt->condition),
				 block_copy(c, parent, while_stmt->code));
}

heck_stmt* copy_stmt_ret(heck_code* c, heck_scope* parent, heck_stmt* stmt)
{
	return create_stmt_ret(c, stmt->fp,
			       copy_expr(c, stmt->value.expr_value));
}
...
void print_stmt_class(heck_stmt* stmt, int indent) { printf("class stmt\n"); }

void print_stmt_func(heck_stmt* stmt, int indent) { printf("function stmt\n"); }

void print_stmt_err(heck_stmt* stmt, int indent) { printf("@error\n"); }