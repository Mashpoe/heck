...
// union bucket_info {
//	// if a bucket's key is null, true means that it is a placeholder bucket
//upon which other buckets depend 	bool tombstone;
//
...
static void str_table_resize(str_table* t)
{
	...
	t->buckets = calloc(t->capacity,
			    sizeof(str_entry)); // initializes everything to 0
	// printf("resize %i\n", t->capacity);
...
/*	returns the address of the bucket (str_obj**) rather than the actual
   bucket (str_obj*) so the bucket can be reassigned */
static str_entry* find_entry(str_table* t, str_entry value)
{
	...
	for (;;)
	{
		...
		if (*entry == NULL)
		{
			...
			// if (!b->info.tombstone) commented out because we
			// don't use tombstones
			return entry;

			// check if the data is the same
		}
		else if ((*entry)->size == value->size &&
			 memcmp((*entry)->value, value->value, value->size) ==
			     0)
		{
			return entry;
		}
...
str_entry str_table_get_entry(str_table* t, str_entry value)
{
	...
	if ((*entry) != NULL)
	{
		...
		/*	free duplicate
			like realloc, it frees the old, unused value and returns
		   the new one */
		if (*entry != value)
		{
	...
	{
		str_table_resize(t);
		// reassign entry, as the str_table was resized and it probably
		// moved
	...
	//	memcpy(entry_value, value, size); // make a copy of the data for
	//the new entry 	entry->value = entry_value;

	// just take ownership of the data, this is only for immutable strings
