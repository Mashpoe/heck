...
inline void step_n(parser* p, int n) { p->pos += n; }

extern heck_token* peek(parser* p);
inline heck_token* peek(parser* p) { return p->code->token_vec[p->pos]; }
...
inline heck_token* next(parser* p) { return p->code->token_vec[p->pos + 1]; }

extern bool at_end(parser* p);
inline bool at_end(parser* p) { return peek(p)->type == TK_EOF; }

extern bool peek_newline(parser* p);
inline bool peek_newline(parser* p) { return peek(p)->ln != next(p)->ln; }
#else
...
heck_data_type* parse_data_type(parser* p, heck_scope* parent)
{
	...
	switch (previous(p)->type)
	{
		case TK_IDF:
		{
			...
			if (match(p, TK_COLON))
			{
				...
				if (!match(p, TK_SQR_L))
				{
					parser_error(
					    p, peek(p), true,
					    "expected a type argument list");
					return NULL;
				}

				t->value.class_type.type_arg_vec =
				    vector_create();
				...
				for (;;)
				{
					...
					heck_data_type* type_arg =
					    (heck_data_type*)parse_data_type(
						p, parent);

					if (type_arg == NULL)
					{
						// we do not need to call
						// panic_mode() because it was
						// called by parse_type_depth
						// don't print error; child type
						// already did
						vector_free(t->value.class_type
								.type_arg_vec);
						free(t);
						return NULL;
					}
		...
		case TK_PRIM_TYPE:
		{
			// override table entry so it doesn't try to free the
			// primitive type
...
{ // assumes an identifier was just found with match(p)
	...
	for (;;)
	{
		...
		/*	don't advance until we know there is a dot followed by
		   an idf this allows the parser to handle the dot on its own */
...
// e.g. (func() {})
heck_expr* parse_func_expr(parser* p, heck_scope* parent) { return NULL; }

heck_expr* primary_idf(parser* p, heck_scope* parent, idf_context context)
{ // assumes an idf was already matched
	heck_file_pos* expr_start = &previous(p)->fp;
	// expr start_must be evaluated before identifier(p)
	heck_expr* idf_expr =
	    create_expr_value(p->code, expr_start, identifier(p), context);

	// function call
	if (match(p, TK_PAR_L))
	{
		heck_expr* call =
		    create_expr_call(p->code, expr_start, idf_expr);
		call->fp = expr_start;
		...
		for (;;)
		{
			vector_add(&call->value.call.arg_vec,
				   expression(p, parent));
...
heck_expr* primary(parser* p, heck_scope* parent)
{
	...
	// if (match(p, TK_KW_NULL)) return create_expr_literal(/* something to
	// represent null */)

	if (match(p, TK_LITERAL))
	{
		return create_expr_literal(p->code, expr_start,
					   previous(p)->value.literal_value);
	}
	...
	{
		// This is the ONLY place where the global and local keywords
		// should be parsed
	...
	while (match(p, TK_SQR_L))
	{
		expr = create_expr_arr_access(p->code, expr_start, expr,
					      expression(p, parent));
		if (!match(p, TK_SQR_R))
		{
			parser_error(p, peek(p), true, "expected ]");
...
// TODO: associate operators with their corresponding vtables during token
// creation
heck_expr* unary(parser* p, heck_scope* parent)
{
	...
	switch (operator)
	{
		...
		// 	step(p);
		// 	const heck_data_type* data_type = parse_data_type(p,
		// parent); 	if (data_type->type_name == TYPE_ERR)
		// return create_expr_err(p->code, expr_start); 	if
		// (data_type->type_name != TYPE_CLASS ||
		// data_type->value.class_type.type_arg_vec == NULL) { if
		// (!match(p, TK_OP_GTR)) { 			parser_error(p,
		// peek(p), true, "unexpected token"); 			return
		// create_expr_err(p->code, expr_start);
		// 		}
		// 	}
		// 	return create_expr_cast(p->code, expr_start, data_type,
		// primary(p, parent));
		// }
		default:
	...
	return create_expr_unary(p->code, expr_start,
				 unary(p, parent), operator, vtable);
}
...
heck_expr* type_cast(parser* p, heck_scope* parent)
{
	...
	for (;;)
	{
		...
		step(p);
		expr = create_expr_cast(p->code, expr_start, expr,
					parse_data_type(p, parent));
	}
}
...
heck_expr* multiplication(parser* p, heck_scope* parent)
{
	...
	for (;;)
	{
		...
		step(p);
		expr = create_expr_binary(p->code, expr_start, expr, operator,
					  unary(p, parent), vtable);
	}
	//	while (match(p, TK_OP_MULT) || match(p, TK_OP_DIV) || match(p,
	// TK_OP_MOD)) { 		heck_tk_type operator =
	// previous(p)->type; heck_expr* right = unary(p); 		expr =
	// create_expr_binary(expr, operator, right);
	//	}
...
heck_expr* addition(parser* p, heck_scope* parent)
{
	...
	for (;;)
	{
		...
		step(p);
		expr = create_expr_binary(p->code, expr_start, expr, operator,
					  multiplication(p, parent), vtable);
	}
...
heck_expr* comparison(parser* p, heck_scope* parent)
{
	...
	for (;;)
	{
		...
		step(p);
		expr = create_expr_binary(p->code, expr_start, expr, operator,
					  addition(p, parent), vtable);
	}

	//	while (match(p, TK_OP_GTR) || match(p, TK_OP_GTR_EQ) || match(p,
	// TK_OP_LESS) || match(p, TK_OP_LESS_EQ)) { 		heck_tk_type
	// operator =
	// previous(p)->type; 		heck_expr* right = addition(p);
	// expr = create_expr_binary(expr, operator, right);
	//	}
...
heck_expr* equality(parser* p, heck_scope* parent)
{
	...
	//		expr = create_expr_binary(expr, operator, right);
	//		//expr->data_type.type_name = TYPE_BOOL; // equality
	// returns a bool
	//	}
}
...
heck_expr* logical_and(parser* p, heck_scope* parent)
{
	...
	{
		expr =
		    create_expr_binary(p->code, expr_start, expr, TK_OP_AND,
				       equality(p, parent), &expr_vtable_and);
	}
...
heck_expr* logical_xor(parser* p, heck_scope* parent)
{
	...
	{
		expr = create_expr_binary(p->code, expr_start, expr, TK_OP_XOR,
					  logical_and(p, parent),
					  &expr_vtable_xor);
	}
...
heck_expr* logical_or(parser* p, heck_scope* parent)
{
	...
	{
		expr =
		    create_expr_binary(p->code, expr_start, expr, TK_OP_OR,
				       logical_xor(p, parent), &expr_vtable_or);
	}
...
heck_expr* ternary(parser* p, heck_scope* parent)
{
	...
	if (match(p, TK_Q_MARK))
	{
		...
		if (!match(p, TK_COLON))
		{
			...
			return create_expr_ternary(
			    p->code, expr_start, expr, value_a,
			    create_expr_err(p->code, expr_start));
		}
		else
		{
			return create_expr_ternary(p->code, expr_start, expr,
						   value_a,
						   expression(p, parent));
		}
	}
...
heck_expr* assignment(parser* p, heck_scope* parent)
{
	...
	if (match(p, TK_OP_ASG))
	{
		...
		if (expr->type == EXPR_VALUE || expr->type == EXPR_ARR_ACCESS)
		{
			...
			heck_expr* asg =
			    create_expr_asg(p->code, expr_start, expr, left);
...
{
	STMT_FLAG_GLOBAL =
	    0, // the global scope, mutually exclusive to other flags
	STMT_FLAG_FUNC = 1,  // inside a function
	STMT_FLAG_LOCAL = 2, // child scope, if statement, etc
	STMT_FLAG_LOOP =
	    4, // inside a structure that supports a break statement
};
...
heck_variable* variable_decl(parser* p, heck_scope* parent)
{
	...
	if (match(p, TK_COLON))
	{
		...
		if (data_type == NULL)
			// no need to report an error because parse_data_type
			// already did
			return NULL;
	}
	else
	...
	// create the variable
	heck_variable* variable = variable_create(
	    p->code, parent, &start_tk->fp, name, data_type, value);
...
{
	return create_stmt_block(
	    p->code, &peek(p)->fp,
	    parse_block(p, scope_create(p->code, parent), flags));
}
...
heck_stmt* if_statement(parser* p, heck_scope* parent, uint8_t flags)
{
	...
	heck_if_node* first_node =
	    create_if_node(p->code, parent, expression(p, parent));
	...
	for (;;)
	{
		...
		if (match(p, TK_KW_IF))
		{
			node->next = create_if_node(p->code, parent,
						    expression(p, parent));
		}
		else
		{
...
bool parse_parameters(parser* p, heck_func_decl* decl)
{
	...
	if (!match(p, TK_PAR_R))
	{
		...
		for (;;)
		{
			...
			// use decl->scope for parent to put parameters in the
			// same scope heck_name* param_name = variable_decl(p,
			// decl->scope); heck_variable* param =
			// param_name->value.var_value;
			...
			// TODO: if param has a default value, do not set
			// generic to true the type can be deduced in
			// func_resolve_name
...
bool parse_operator_decl(parser* p, heck_operator_type* type_out,
			 heck_func_decl* decl)
{
	...
	else
	{
		...
		const heck_data_type* conversion_type =
		    parse_data_type(p, decl->scope->parent);
...
void parse_func_def(parser* p, heck_scope* parent)
{
	...
	heck_idf func_idf;
	heck_name* func_name; // can be either a function or a class in the case
			      // of operator overloading ??
	heck_scope* func_scope; // the parent scope of the function (function
				// names don't have child scopes)
	if (match(p, TK_IDF))
	{
		...
		{
			parser_error(p, peek(p), true,
				     "unable to create function {I}", func_idf);
			return;
		}
	...
	heck_func* func = NULL;
	// heck_func_list* overload_list = NULL; // the list that we add the
	// func to
	...
	if (func_idf == NULL || match(p, TK_DOT))
	{
		...
		{
			parser_error(p, peek(p), true,
				     "expected a function name or operator "
				     "overload type");
			return;
		}
		...
		{
			parser_error(
			    p, previous(p), true, "{s} overload outside class",
			    previous(p)->type == TK_KW_OPERATOR ? "operator"
								: "conversion");
			return;
		}
		...
		add_operator_def(operator_class->value.class_value,
				 &operator_type, func);
	}
	else
	{
		...
		if (func_name->type == IDF_UNDECLARED)
		{
			...
			if (func_name->child_scope != NULL)
			{
				...
				parser_error(
				    p, peek(p), true,
				    "unable to create function \"{s}\" in "
				    "place of implicitly declared item",
				    func_idf);
				return;
			}
		...
		{
			parser_error(p, peek(p), true,
				     "unable to create function with the same "
				     "name as {s} \"{I}\"",
				     get_idf_type_string(func_name->type),
				     func_idf);
			return;
		}
	...
	if (peek(p)->type == TK_BRAC_L)
	{
		...
		if (func->value.code->type == BLOCK_MAY_RETURN)
		{
			parser_error(
			    p, peek(p), false,
			    "function \"{I}\" only returns in some cases",
			    func_idf);
		}
	}
	else
	{
		...
		vector_add(&func->value.code->stmt_vec,
			   create_stmt_err(p->code, &peek(p)->fp));
...
heck_stmt* ret_statement(parser* p, heck_scope* parent)
{
	...
	// expression must start on the same line as return statement or else
	// it's void
...
void parse_class_def(parser* p, heck_scope* parent)
{
	...
	if (class_name == NULL)
	{
		parser_error(p, peek(p), true, "unable to create class \"{I}\"",
			     class_idf);
		return;
	}
	...
	if (class_name->type == IDF_UNDECLARED)
	{
		...
		// assume things will be declared in the class, create a scope
		// and names map
		...
		// if class_idf has one value (e.g. name instead of
		// classA.classB.name)
		if (class_idf[1] == NULL)
		{
			class_name->type = IDF_CLASS;
	...
	else if (class_name->type == IDF_CLASS)
	{
		...
		{
			parser_error(p, start_tk, true,
				     "redefinition of class \"{I}\"",
				     class_idf);
			return;
		}

		// another class definition exists, but there is no class
		// declaration
	}
	else if (class_name->type == IDF_UNDECLARED_CLASS)
	{
		parser_error(p, start_tk, true, "redefinition of class \"{I}\"",
			     class_idf);
		return;
	}
	else
	{
		parser_error(
		    p, start_tk, true,
		    "unable to create class with the same name as {s} \"{I}\"",
		    get_idf_type_string(class_name->type), class_idf);
		return;
	}
	...
	else if (idf_map_get(parent->names, name_str, (void**)&func_name))
	{
		...
		if (func_name->type == IDF_FUNCTION)
		{
			...
			if (func_name->value.func_value.decl_vec == NULL)
				func_name->value.func_value.decl_vec =
				    vector_create();
		}
		...
		else
		{
			...
			parser_error(p, peek(p), true,
				     "cannot declare function with the same "
				     "name as {s} \"{s}\"",
				     get_idf_type_string(func_name->type),
				     name_str->value);
			return NULL;
...
void parse_func_extern(parser* p, heck_scope* parent)
{
	...
	else
	{
		...
		{
			parser_error(p, start_tk, true,
				     "{s} overload outside class definition",
				     previous(p)->type == TK_KW_OPERATOR
					 ? "operator"
					 : "conversion");
			return;
		}
		heck_class* operator_class =
		    parent->parent_class->value.class_value;
		...
		// add the operator overload to the class
		add_operator_decl(operator_class, &operator_type,
				  &operator_decl);
	}
}
...
void import_func(parser* p, heck_scope* parent)
{
	...
	{
		parser_error(p, peek(p), true,
			     "function import outside of global scope");
		return;
	}
...
void import_file(parser* p, heck_scope* parent)
{
	...
	if (import_tk->type != TK_LITERAL ||
	    import_tk->value.literal_value->data_type != data_type_string)
	{
	...
	{
		parser_error(p, previous(p), false,
			     "import outside of the global scope");
		step(p);
		return;
	}
	step(p);
	...
	{
		parser_error(p, peek(p), false, "unable to open file \"{s}\"",
			     filename->value);
	}
}
...
void import(parser* p, heck_scope* parent)
{
	...
	else
	{
		...
		// // TODO: add other extern declaration types
		// parser_error(p, peek(p), true, "expected a function
		// declaration");
	}
}
...
heck_stmt* parse_namespace(parser* p, heck_scope* parent)
{
	...
	{
		parser_error(p, peek(p), true,
			     "error: unable to create namespace \"{I}\"",
			     nmsp_idf);
		return create_stmt_err(p->code, &start_tk->fp);
	}
	...
	if (nmsp->type != IDF_NAMESPACE)
	{
		if (nmsp->type != IDF_UNDECLARED)
		{
			...
			parser_error(p, peek(p), false,
				     "unable to create namespace with the same "
				     "name as {s} \"{I}\"",
...
void parse_statement(parser* p, heck_block* block, uint8_t flags)
{
	...
	switch (t->type)
	{
		...
		case TK_BRAC_L:
			...
			break;
		default:
			stmt = create_stmt_expr(p->code, &peek(p)->fp,
						expression(p, block->scope));
	}
